...
export default function render(container) {
        ...
        <div class="fuel-calculator">
            ...
            <div class="fc-section">
                ...
                <div class="fc-input-group">
                    <input type="number" id="fc-distance" class="fc-input" value="100" step="any">
                    <select id="fc-distance-unit" class="fc-select-small">
            ...
            <div class="fc-section">
                ...
                <div class="fc-input-group">
                    <input type="number" id="fc-consumption" class="fc-input" value="7.5" step="any">
                    <select id="fc-consumption-unit" class="fc-select-small">
            ...
            <div class="fc-section">
                ...
                <div class="fc-input-group">
                    <input type="number" id="fc-price" class="fc-input" value="38.50" step="any">
                    <select id="fc-currency" class="fc-select-small">
            ...
            <!-- Počet cestujících -->
            <div class="fc-section">
                <label class="fc-label">👥 Počet cestujících (pro sdílení nákladů)</label>
                <div class="fc-input-group">
                    <input type="number" id="fc-passengers" class="fc-input" value="1" min="1" step="1">
                </div>
            </div>

            <!-- Tlačítka -->
            <div class="fc-buttons">
                <button id="fc-calculate" class="fc-btn fc-btn-primary">⚡ Spočítat cestu</button>
                <button id="fc-clear" class="fc-btn fc-btn-secondary">🗑️ Vyčistit</button>
            </div>
            ...
            <div class="fc-results-section">
                ...
                <div id="fc-results" class="fc-results">
                    <div class="fc-empty">Vyplň údaje a klikni na "Spočítat cestu"</div>
                </div>
            </div>

            <!-- Tip -->
            <div class="fc-tip">
                💡 <strong>Tip:</strong> Můžeš zadat vzdálenost v km nebo mílích, spotřebu v l/100km nebo mpg. Výsledky se přepočítají automaticky.
            </div>
        </div>
    `;
    ...
    function convertDistance(km) {
        return distanceUnit.value === 'km' ? km : km * 1.60934;
    }
    ...
    function calculate() {
        ...
        if (distance <= 0) {
            resultsDiv.innerHTML = '<div class="fc-error">❌ Zadej platnou vzdálenost</div>';
            return;
        }
        if (consumption <= 0) {
            resultsDiv.innerHTML = '<div class="fc-error">❌ Zadej platnou spotřebu</div>';
            return;
        }
        if (price <= 0) {
            resultsDiv.innerHTML = '<div class="fc-error">❌ Zadej platnou cenu paliva</div>';
            return;
        }

        // Převod na metrické jednotky
        const distanceKm = distanceUnit.value === 'km' ? distance : distance * 1.60934;
        ...
        resultsDiv.innerHTML = `
            ...
            <div class="fc-result-card">
                <div class="fc-result-icon">📏</div>
                <div class="fc-result-content">
                    <div class="fc-result-value">${costPerKm.toFixed(2)} ${currencySymbol}/km</div>
                    <div class="fc-result-label">Cena na kilometr</div>
                </div>
            </div>
    ...
    function clearAll() {
        ...
        consumptionInput.value = '7.5';
        priceInput.value = '38.50';
        passengersInput.value = '1';
        distanceUnit.value = 'km';
        consumptionUnit.value = 'l100km';
        currencySelect.value = 'czk';
        resultsDiv.innerHTML = '<div class="fc-empty">Vyplň údaje a klikni na "Spočítat cestu"</div>';
    ...
    // Auto-výpočet při změně (debounce)
    let debounceTimer;
    const autoCalculate = () => {
    ...
    function loadSettings() {
        distanceInput.value = storage.get('distance', '100');
        distanceUnit.value = storage.get('distanceUnit', 'km');
        consumptionInput.value = storage.get('consumption', '7.5');
        consumptionUnit.value = storage.get('consumptionUnit', 'l100km');
        priceInput.value = storage.get('price', '38.50');
